...
class ElevatorWithQueueTest extends AnyFunSuite with BeforeAndAfter {
  ...
  test("Update with multiple floors UP test") {
    // Given
    val destinationFloorList = List(2, 1, 5, -2)
    ...
    // Then
    assert(elevatorWithQueue.status() == ElevatorStatus(elevatorID = elevatorId, currentFloor = 1, destinationFloor = 2))
  }

  test("Update with multiple floor with level smaller and higher than current test") {
    // Given
    val destinationFloorList = List(2, -2)

    // When
    destinationFloorList.foreach(floor => elevatorWithQueue.update(floor))
    (0 until 2).foreach(_ => elevatorWithQueue.step())

    // Then
    assert(elevatorWithQueue.status() == ElevatorStatus(elevatorID = elevatorId, currentFloor = 2, destinationFloor = -2))
  }
